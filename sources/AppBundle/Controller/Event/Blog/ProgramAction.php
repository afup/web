...
final class ProgramAction extends AbstractController
{
    public function __construct(
        ...
        private readonly TalkRepository    $talkRepository, private readonly EventThemeRepository $eventThemeRepository,
    ) {}
    ...
    public function __invoke(Request $request, $eventSlug): Response
    {
        ...
        if ($event->getHasThemes()) {
            ...
            usort($themes, fn ($a, $b) => $a->getName() <=> $b->getName());
            $themes = array_combine(array_map(fn ($theme) => $theme->getId(), $themes), $themes);
        }
        $now = new \DateTime();
