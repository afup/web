...
class InvoicingPdfGenerator
{
    ...
    {
        return 'Facture - ' . $invoicing->societe . ' - ' . ($invoicing->dateFacture ? $invoicing->dateFacture->format('Y-m-d') : '') . '.pdf';
    }

    public function getQuotationFilename(Invoicing $invoicing): string
    {
        return 'Devis - ' . $invoicing->societe . ' - ' . ($invoicing->dateDevis ? $invoicing->dateDevis->format('Y-m-d') : '') . '.pdf';
    }
