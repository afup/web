 ...
 * todolist:
 * - poster à 13h
 * - poster le mardi et le jeudi et pas les ven/sam/dim
 */
final class Engine
{
    ...
    public function run(?string $eventPath = null): ?HistoryEntry
    {
        $talk = $this->pickRandomTalk($eventPath);
    ...
    private function pickRandomTalk(?string $eventPath = null): ?Talk
    {
        $selectedEvent = null;

        if (null !== $eventPath) {
            $selectedEvent = $this->eventRepository->getByPath($eventPath);

            if (null === $selectedEvent) {
                throw new \InvalidArgumentException("L'évènement $eventPath n'existe pas");
            }
        }
        ...
        foreach ($plannings as $planning) {
            // Les keynotes ne sont pas postées
            if ($planning->getIsKeynote()) {
                continue;
            }
            ...
            // On peut trier sur un event en particulier
            if (null !== $selectedEvent && $planning->getEventId() !== $selectedEvent->getId()) {
                continue;
            }
            ...
            // Si on ne trie pas sur un event spécifique, on trie sur la date pour éviter les talks trop anciens
