...
3. Lancer `make local:up` pour lancer le projet
4. Lancer `make local:init` pour le setup initial (config, dépendances, base de données)
...
Pour faciliter leur configuration, un fichier `.env` est créé à la racine du projet à la première exécution de la commande `make local:up`.
...
La variable d'environnement `DOCKER_UP_OPTIONS` permet de passer des options à la commande `docker compose up` (via `make local:up`).
...
> [!NOTE]
> Une commande `make docker:*` doit être exécutée dans le container (connectez-vous via `make local:console`
> puis lancez `make docker:*`), tandis qu'une commande `make local:*` doit être exécutée depuis votre machine
> (elle orchestre docker pour vous).

Il faut se connecter au container via `make local:console` puis :
...
Une alternative est d'utiliser la commande `make local:test-functional`, attention cette commande arrête les containeurs de
...
Une alternative est d'utiliser `make docker:test` dans le container, attention cette commande
...
Pour les lancer, il faut se connecter au container via `make local:console` puis :

```bash
./bin/phpunit --testsuite integration
```

Une alternative est d'utiliser la commande `make docker:test-integration`, attention cette commande arrête les containeurs de
tests à la fin de l'exécution de la suite de test.
