Feature: Administration - Trésorerie - Devis/Facture
  ...
  Scenario: Créer/Modifier un devis, une facture
    ...
    When I fill in "quotation[societe]" with "ESN Corp"
    And I fill in "quotation[adresse]" with "3 rue du chemin"
    And I fill in "quotation[ville]" with "Dijon"
    And I fill in "quotation[codePostal]" with "21000"
    And I fill in "quotation[email]" with "martine@ens-corp.biz"
    And I click on link with class "add_item_link"
    ...
    And I fill in "quotation[details][0][quantite]" with "12"
    And I fill in "quotation[details][0][prixUnitaire]" with "660"
    ...
    Then I should see "Ajouter un devis"
    When I fill in "quotation[societe]" with "ESN dev en folie"
    And I fill in "quotation[service]" with "Développement"
    And I fill in "quotation[adresse]" with "1 rue du chemin"
    And I fill in "quotation[codePostal]" with "75000"
    And I fill in "quotation[ville]" with "Paris"
    And I fill in "quotation[nom]" with "Moreau"
    And I fill in "quotation[prenom]" with "Martine"
    And I fill in "quotation[telephone]" with "0101010101"
    And I fill in "quotation[email]" with "martine@ens-en-folie.biz"
    And I fill in "quotation[tvaIntra]" with "FR7612345"
    And I fill in "quotation[referenceClient1]" with "CLIENT-AFGD5S"
    And I fill in "quotation[referenceClient2]" with "AFGD5S"
    And I fill in "quotation[referenceClient3]" with "000AFGD5S"
    ...
    And I fill in "quotation[details][1][quantite]" with "1"
    And I fill in "quotation[details][1][prixUnitaire]" with "12000"
    When I press "Ajouter" and wait until I see "L'écriture a été ajoutée"
    ...
    And I fill in "quotation[ville]" with "Paris Cedex 1"
    ...
    When I follow the button of tooltip "Modifier le devis ESN dev en folie"
    Then I should see "Dupliquer le devis"
    When I follow "Dupliquer le devis"
    Then I should see "Ajouter un devis"
    And the "quotation[societe]" field should contain "ESN dev en folie"
    And the "quotation[adresse]" field should contain "1 rue du chemin"
    And the "quotation[email]" field should contain "martine@ens-en-folie.biz"
    And the "quotation[details][0][reference]" field should contain "COACH-001"
    And the "quotation[details][1][reference]" field should contain "ARCHI-007"
    When I fill in "quotation[societe]" with "ESN copie folie"
    And I press "Ajouter" and wait until I see "L'écriture a été ajoutée"
    Then I should see "ESN copie folie"
    # Suppression de toutes les lignes sauf une dans le devis dupliqué (test suppression multiple)
    When I follow the button of tooltip "Modifier le devis ESN copie folie"
    Then I click on link with id "remove_row_0"
    Then I click on link with id "remove_row_1"
    Then I click on link with class "add_item_link"
    And I fill in "quotation[details][2][reference]" with "UNIQUE-001"
    And I fill in "quotation[details][2][designation]" with "Ligne unique"
    And I fill in "quotation[details][2][quantite]" with "1"
    And I fill in "quotation[details][2][prixUnitaire]" with "500"
    When I press "Modifier" and wait until I see "L'écriture a été modifiée"
    Then I should see "L'écriture a été modifiée"
    And I should see "500,00"
    # Suppression d'une ligne dans le devis original
    When I follow the button of tooltip "Modifier le devis ESN dev en folie"
    Then I click on link with id "remove_row_0"
    When I press "Modifier" and wait until I see "L'écriture a été modifiée"
    Then I should see "L'écriture a été modifiée"
    ...
    Then I fill in "invoice[ville]" with "Paris Cedex 7"
    And I select "1" from "invoice[etatPaiement]"
    And I fill in "invoice[datePaiement]" with "2026-12-31"
    When I press "Modifier" and wait until I see "L'écriture a été modifiée"
  ...
  Scenario: Test du PDF de facture avant 2024
    ...
    Then The page "1" of the PDF should contain "Type Description\tQuantitePrix Total"
    Then The page "1" of the PDF should contain "forum_php_2023Forum PHP 2023 - Sponsoring Bronze 1.00 1000.00 € 1000 €"
  ...
  Scenario: Test du PDF de facture après 2024
    ...
    Then The page "1" of the PDF should contain "Comme convenu, nous vous prions de trouver votre facture"
    Then The page "1" of the PDF should contain "Type Description\tQuantiteTVA\tPrix HT Total TTC"
    Then The page "1" of the PDF should contain "forum_php_2025Forum PHP 2025 - Sponsoring"
    Then The page "1" of the PDF should contain "Bronze\n1.00 20.00%1 000,00 € 1 200,00 €"
