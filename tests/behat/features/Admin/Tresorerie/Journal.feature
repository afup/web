Feature: Administration - Trésorerie - Journal
  ...
  Scenario: Journal dépense/recette
    ...
    # Ajout d'une dépense
    When I follow "Ajouter"
    And I select "1" from "transaction[operation]"
    And I select "1" from "transaction[compte]"
    And I select "5" from "transaction[evenement]"
    And I fill in "transaction[dateEcriture]" with "2026-01-01"
    And I select "26" from "transaction[categorie]"
    And I fill in "transaction[nomFournisseur]" with "Fournisseur"
    And I fill in "transaction[tvaIntra]" with "FR4412345"
    And I select "france" from "transaction[tvaZone]"
    And I fill in "transaction[numero]" with "001"
    And I fill in "transaction[description]" with "Description dépense 001"
    And I fill in "transaction[montant]" with "12000"
    And I fill in "transaction[commentaire]" with "Commentaire dépense 001"
    And I press "Ajouter"
    ...
    And I fill in "transaction[montant]" with "12001"
    ...
    # Ajout d'une recette
    When I follow "Ajouter"
    And I select "2" from "transaction[operation]"
    And I select "1" from "transaction[compte]"
    And I select "5" from "transaction[evenement]"
    And I fill in "transaction[dateEcriture]" with "2026-01-02"
    And I select "26" from "transaction[categorie]"
    And I fill in "transaction[nomFournisseur]" with "Fournisseur"
    ...
    And I fill in "transaction[montant]" with "12000"
    And I fill in "transaction[commentaire]" with "Commentaire recette 001"
    And I press "Ajouter"
    Then I should see "L'écriture a été ajoutée"
    ...
    And I fill in "transaction[montant]" with "12002"
    When I press "Modifier"
    Then I should see "L'écriture a été modifiée"
