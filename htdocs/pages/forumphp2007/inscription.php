...
for ($i=1;$i <= $nombre_personnes;$i++)
{
	...
	$groupe = array();
	$groupe[] = &HTML_QuickForm::createElement('radio', 'type_inscription'.$i, null, 'Journée fonctionnelle, mercredi 21 novembre 2007 : <strong>' . $AFUP_Tarifs_Forum[AFUP_FORUM_PREMIERE_JOURNEE] . ' ' . EURO . '</strong>', AFUP_FORUM_PREMIERE_JOURNEE);
	$groupe[] = &HTML_QuickForm::createElement('radio', 'type_inscription'.$i, null, 'Journée technique, jeudi 22 novembre 2007 : <strong>' . $AFUP_Tarifs_Forum[AFUP_FORUM_PREMIERE_JOURNEE] . ' ' . EURO . '</strong>' , AFUP_FORUM_DEUXIEME_JOURNEE);
...
if ($formulaire->validate()) {
    ...
    if ($probleme == 1) {
        ...
        if ($valeurs['type_reglement'] == AFUP_FORUM_REGLEMENT_CARTE_BANCAIRE) {
            ...
            $paybox->set_cmd($valeurs['reference']); // Référence de la commande
            $paybox->set_porteur($valeurs['email_facturation']); // Email du client final (Le porteur de la carte)
