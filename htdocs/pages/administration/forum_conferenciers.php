...
if ($action == 'inscrire_forum') {
    ...
    if ($action == 'modifier') {
        if (intval($champs['id_forum']) < ID_FORUM_PHOTO_STORAGE) {
            ...
            $formulaire->addElement('static', 'html', '', sprintf('<a href="%s">Voir la photo originale</a>', $storage->getUrl($speaker, \AppBundle\CFP\PhotoStorage::DIR_ORIGINAL)));
        }
    }
    ...
    $smarty->assign('sessions_conferencier', $sessions);
