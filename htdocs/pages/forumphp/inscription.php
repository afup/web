...
if ($formulaire->validate()) {
    ...
    if (!$ok) {
        ...
        if ($valeurs['type_reglement'] == AFUP_FORUM_REGLEMENT_CARTE_BANCAIRE) {
            ...
            $paybox->set_total($tarifs[$valeurs['type_inscription']] * 100); // Total de la commande, en centimes d'euros
            $paybox->set_cmd($reference); // Référence de la commande
            $paybox->set_porteur($valeurs['email']); // Email du client final (Le porteur de la carte)
