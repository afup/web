...
{
 $frm = $_GET;
	$sql="DELETE FROM compta_operation WHERE 
				 id = '".$frm['id']."' 
		";
...
switch($frm[action])
{
	...
	/*	if ( empty($frm['reglement']) ) 
		...
		$sql="UPDATE compta_operation SET 
...
/*		if ( empty($frm['reglement']) ) 
		 		 	...
		 		 	|| $_POST['idclef'] != $_SESSION['idclef'])
		{
		 ...
		 $sql = "INSERT INTO compta_operation (`operation`)
		 		VALUES (
...
if ($nrow[0] !='')
{
	echo "<td><b>Journal Disponible</b><br>";
	echo "<table>";
	$qid=$cnx->prepare($sql);
	$qid->execute();
...
/*$sql="SELECT rubrique.id as idselect,rubrique.iduser,rubrique.nom,user.id  
		...
		WHERE rubrique.iduser=user.id 
			...
			AND rubrique.id='$id' 
	";*/
$sql="SELECT * FROM compta_operation WHERE id='$id' ";	
