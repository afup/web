...
echo "<td colspan='2' align='center'>".fprix($montant_bal_recette-$montant_bal_depense)."</td>";
echo "</tr>";
...
detail ($idevenement,$idcategorie,$periode_debut,$periode_fin,$idperiode);
...
echo recup_evenement($idevenement)."<br>";
	echo "<table border='1' width='100%' cellspacing='0' cellpadding='2'>";
echo "<tr><td>Date ecriture</td><td>Categorie</td><td>Description</td><td>Depense</td><td>Recette</td></tr>";
$cnx2 = new PDO('mysql:host='.$serveur.';port='.$port.';dbname='.$bdd, $user, $passwd);
/*$sql2="SELECT * 
	   FROM compta  
	   WHERE idevenement=$idevenement  AND date_ecriture>='$periode_debut' AND date_ecriture<='$periode_fin' 
	  ORDER BY idevenement,idcategorie";
*/
$sql2="SELECT compta.*,compta_categorie.id,compta_categorie.categorie   
	FROM compta, compta_categorie  
	WHERE compta.idevenement=$idevenement  AND compta.date_ecriture>='$periode_debut' AND compta.date_ecriture<='$periode_fin' 
		  AND compta.idcategorie = compta_categorie.id     
	ORDER BY compta.idevenement,compta.idcategorie";
