...
{
 $frm = $_GET;
	$sql="DELETE FROM budget_evenement WHERE 
				 id = '".$frm['id']."' 
		";
...
switch($frm[action])
{
	...
	/*	if ( empty($frm['reglement']) ) 
		...
		$sql="UPDATE budget_evenement SET 
...
/*		if ( empty($frm['reglement']) ) 
		 		 	...
		 		 	|| $_POST['idclef'] != $_SESSION['idclef'])
		{
		 ...
		 $sql = "INSERT INTO budget_evenement (`evenement`)
		 		VALUES (
...
/*$sql="SELECT rubrique.id as idselect,rubrique.iduser,rubrique.nom,user.id  
		...
		WHERE rubrique.iduser=user.id 
			...
			AND rubrique.id='$id' 
	";*/
$sql="SELECT * FROM budget_evenement WHERE id='$id' ";	
