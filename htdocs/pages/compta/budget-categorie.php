...
{
 $frm = $_GET;
	$sql="DELETE FROM  budget_categorie WHERE 
				 id = '".$frm['id']."' 
		";
...
switch($frm[action])
{
	...
	/*	if ( empty($frm['reglement']) ) 
		...
		$sql="UPDATE budget_categorie SET 
...
/*		if ( empty($frm['reglement']) ) 
		 		 	...
		 		 	|| $_POST['idclef'] != $_SESSION['idclef'])
		{
		 ...
		 $sql = "INSERT INTO budget_categorie (`categorie`)
		 		VALUES (
...
if ($nrow[0] !='')
{
	echo "<td><b>Categories Disponible pour le budget</b><br>";
	echo "<table>";
	$qid=$cnx->prepare($sql);
	$qid->execute();
...
/*$sql="SELECT rubrique.id as idselect,rubrique.iduser,rubrique.nom,user.id  
		...
		WHERE rubrique.iduser=user.id 
			...
			AND rubrique.id='$id' 
	";*/
$sql="SELECT * FROM budget_categorie WHERE id='$id' ";	
