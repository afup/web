...
switch($frm[action])
{
	...
	/*	if ( empty($frm['reglement']) ) 
		...
		$sql="UPDATE compta_simulation SET 
...
//$periode=$rep[0];
//$idperiode=$frm[idperiode];   
		 $sql = "INSERT INTO compta_simulation (
		 		`idoperation`
		 		,`idcategorie`
		 		,`montant_theo`
		 		,`description`
...
if ($id)
{
$sql="SELECT * FROM compta_simulation WHERE id='$id' ";	
assert ('$cnx->prepare($sql)');
$qid=$cnx->prepare($sql);
$qid->execute();
...
<hr>
<?php
$sql="SELECT compta_simulation.*, budget_categorie.categorie, budget_evenement.evenement,budget_operation.operation  
FROM budget_operation INNER JOIN (
		budget_evenement INNER JOIN (
		budget_categorie INNER JOIN compta_simulation ON budget_categorie.id = compta_simulation.idcategorie) 
		ON budget_evenement.id = compta_simulation.idevenement) 
		ON budget_operation.id = compta_simulation.idoperation
WHERE compta_simulation.periode>='$periode_debut' AND compta_simulation.periode<='$periode_fin'  
ORDER BY compta_simulation.idoperation, budget_evenement.evenement,budget_categorie.categorie,compta_simulation.description  
"; 
