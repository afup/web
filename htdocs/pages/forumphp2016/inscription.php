...
/*if (isset($_GET['nolayout']) === false) {
    ...
    header('Location:http://event.afup.org/forum-php-2016/tickets-inscriptions/');
}*/
  ...
  $formulaire->setDefaults(array(
    ...
    if ($is_prevente) {
        ...
        $groupe[] = &HTML_QuickForm::createElement(
            ...
            $translator->trans('Journée du vendredi 28 octobre 2016 :') . ' <strong>' . $AFUP_Tarifs_Forum[AFUP_FORUM_PREMIERE_JOURNEE] . ' ' . EURO . '</strong>',
            AFUP_FORUM_DEUXIEME_JOURNEE,
            ['data-price' => $AFUP_Tarifs_Forum[AFUP_FORUM_DEUXIEME_JOURNEE], 'data-label' => $translator->trans('Journée du vendredi 28 octobre 2016 :')]
        );
    }
    ...
    if ($probleme == 1) {
        ...
        if ($valeurs['type_reglement'] == AFUP_FORUM_REGLEMENT_CARTE_BANCAIRE) {
            ...
            if (isset($_GET['repondre_a']) === true) {
                ...
                $paybox->set_effectue('https://event.afup.org/inscription-confirmee/');
                $paybox->set_refuse('https://event.afup.org/inscription-refusee/');
                $paybox->set_annule('https://event.afup.org/inscription-annulee/');
                $paybox->set_erreur('https://event.afup.org/erreur-de-paiement/');
            } else {
