...
for ($i=1; $i <= $nombre_personnes; $i++) {
  ...
  if ($is_prevente) {
    ...
    $groupe[] = $formulaire->createElement('radio', 'type_inscription'.$i, null, 'Journée du jeudi 23 octobre 2014 : <strong>' . $AFUP_Tarifs_Forum[AFUP_FORUM_PREMIERE_JOURNEE] . ' ' . EURO . '</strong>', AFUP_FORUM_PREMIERE_JOURNEE);
    $groupe[] = $formulaire->createElement('radio', 'type_inscription'.$i, null, 'Journée du vendredi 24 octobre 2014 : <strong>' . $AFUP_Tarifs_Forum[AFUP_FORUM_PREMIERE_JOURNEE] . ' ' . EURO . '</strong>' , AFUP_FORUM_DEUXIEME_JOURNEE);
  }
  else  {
...
$groupe[] = $formulaire->createElement('radio', 'type_reglement', null, 'Chèque (des frais de traitement de 25€ seront ajoutés à la facture)'        , AFUP_FORUM_REGLEMENT_CHEQUE);
